...
```mermaid
flowchart TD
    START["遍历 os.environ.items()"] --> PASS{"命中 env_passthrough<br>白名单?"}

    PASS -->|是| KEEP_PASS["加入 child_env"]
    PASS -->|否| SECRET{"变量名包含敏感关键词?"}

    SECRET -->|是| DROP_SECRET["跳过，不传递给子进程"]
    SECRET -->|否| PREFIX{"命中安全前缀?"}

    PREFIX -->|是| KEEP_PREFIX["加入 child_env"]
    PREFIX -->|否| DROP_OTHER["跳过"]

    KEEP_PASS --> EXTRA1["追加 HERMES_RPC_SOCKET"]
    KEEP_PREFIX --> EXTRA1
    DROP_SECRET --> EXTRA1
    DROP_OTHER --> EXTRA1

    EXTRA1 --> EXTRA2["设置 PYTHONDONTWRITEBYTECODE=1"]
    EXTRA2 --> EXTRA3["重写 PYTHONPATH，注入 hermes-agent 根目录"]
    EXTRA3 --> TZ{"存在 HERMES_TIMEZONE?"}
    TZ -->|是| ADD_TZ["设置 TZ"]
    TZ -->|否| HOME_CHECK{"存在 profile subprocess HOME?"}
    ADD_TZ --> HOME_CHECK

    HOME_CHECK -->|是| ADD_HOME["覆盖 HOME 为 get_subprocess_home()"]
    HOME_CHECK -->|否| BUILD["完成 child_env 构建"]
    ADD_HOME --> BUILD
```
...
flowchart TD
    ...
    IN --> TRUNC["Head/Tail 截断<br/>上限 50 KB<br/>40% Head + 60% Tail"]
    TRUNC --> ANSI["ANSI 转义序列清洗<br/>ECMA-48 全覆盖"]
    ANSI --> REDACT["凭证脱敏<br/>API Key / Token / 私钥<br/>DB 连接串 / 手机号"]
    REDACT --> JSON["构建 JSON 响应<br/>status + output<br/>tool_calls_made + duration"]
    JSON --> OUT(["返回给 LLM"])
```
...
flowchart TD
    ...
    CREATED["Created<br/>subprocess.Popen()"]
    CREATED --> RUNNING

    RUNNING["Running<br/>进程启动<br/>轮询 proc.poll()"]
    RUNNING -->|"正常退出"| EXITED["Exited<br/>正常退出"]
    RUNNING -->|"time.monotonic() > deadline"| TIMEOUT["Timeout<br/>超时"]

    TIMEOUT --> SIGTERM["SIGTERM<br/>_kill_process_group()"]
    SIGTERM --> WAIT["WaitExit<br/>proc.wait(5s)"]

    WAIT -->|"5s 内退出"| CLEANEXIT["CleanExit<br/>已退出"]
    WAIT -->|"超时未退出"| SIGKILL["SIGKILL<br/>强制终止"]
    SIGKILL --> CLEANEXIT

    EXITED --> CLEANUP
    CLEANEXIT --> CLEANUP

    CLEANUP["Cleanup<br/>删除临时目录 + Socket"]
    CLEANUP --> DONE(["结束"])
```
