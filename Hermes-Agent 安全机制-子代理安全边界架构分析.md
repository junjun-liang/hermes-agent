...
```mermaid
flowchart TD
    START(["LLM 调用 delegate_task()"]) --> PA

    PA{"parent_agent 存在？"}
    PA -->|"否"| E1["❌ 返回错误\n'No parent agent context'"]
    PA -->|"是"| D1

    subgraph VALIDATE["验证阶段"]
        D1{"depth >= MAX_DEPTH=2 ？"}
        D1 -->|"是"| E2["❌ 返回错误\n'Delegation depth limit reached'"]
        D1 -->|"否"| D2
        D2{"goal / tasks 非空？"}
        D2 -->|"否"| E3["❌ 返回错误\n'Goal or tasks required'"]
        D2 -->|"是"| D3
        D3{"len(tasks) <= max_children=3 ？"}
        D3 -->|"否"| E4["❌ 返回错误\n'Too many tasks'"]
        D3 -->|"是"| CFG
    end

    subgraph CRED["凭证解析"]
        CFG["加载 delegation config.yaml"]
        CFG --> RC["_resolve_delegation_credentials()\n返回 {provider, base_url, api_key, model}"]
        RC --> PROV{"delegation.provider 指定？"}
        PROV -->|"是"| RP["解析 provider 凭证\n(base_url / api_key)"]
        PROV -->|"否"| IP["继承父代理凭证"]
        RP & IP --> FILTER
    end

    subgraph BUILD["子代理构建"]
        FILTER["_strip_blocked_tools()\n移除: delegation / clarify / memory / code_execution"]
        FILTER --> SAVE["保存父代理\n_last_resolved_tool_names"]
        SAVE --> LOOP["遍历 task_list"]
        LOOP --> BCA["_build_child_agent()\n创建 AIAgent\n工具集 / 凭证 / 深度+1"]
        BCA --> MORE{"还有任务？"}
        MORE -->|"是"| LOOP
        MORE -->|"否"| EXEC
    end
    ...
    subgraph EXECUTE["执行阶段"]
        EXEC{"n_tasks == 1 ？"}
        EXEC -->|"是"| SINGLE["直接调用\n_run_single_child()"]
        EXEC -->|"否"| POOL["ThreadPoolExecutor\n并发执行最多 3 个"]
        SINGLE & POOL --> COLLECT["收集所有子代理结果"]
    end

    COLLECT --> RESTORE["恢复父代理\n_last_resolved_tool_names"]
    RESTORE --> FMT["格式化响应\nresults + total_duration + total_api_calls"]
    FMT --> DONE(["返回 JSON 结果数组"])
```
                                        │
                                        ▼
                              ┌─────────────────────┐
...
```mermaid
flowchart TD
    START(["_resolve_delegation_credentials\ncfg, parent_agent"]) --> CP

    subgraph RESOLVE["凭证解析"]
        CP{"cfg.provider 非空？"}
        CP -->|"否"| CM{"cfg.model 非空？"}
        CM -->|"否"| RN["返回全 None\n子代理继承父代理凭证"]
        CM -->|"是"| RM["返回 model\n其余字段为 None"]
        CP -->|"是"| BU{"cfg.base_url 指定？"}
        BU -->|"是"| RC1["provider = 'custom'\n使用 cfg.base_url / cfg.api_key"]
        BU -->|"否"| RRP["调用 resolve_runtime_provider()"]
        RRP --> RES{"解析成功？"}
        RES -->|"否"| VE1["❌ 抛出 ValueError"]
        RES -->|"是"| AK{"api_key 非空？"}
        AK -->|"否"| VE2["❌ 抛出 ValueError"]
        AK -->|"是"| RC2["使用 resolved provider 凭证"]
        RC1 & RC2 & RN & RM --> CRED_DICT["返回凭证字典\n{provider, base_url, api_key, api_mode, model}"]
    end

    subgraph LEASE["凭证租赁 (_run_single_child)"]
        CRED_DICT --> POOL{"child._credential_pool 存在？"}
        POOL -->|"否"| SKIP["跳过租赁\n使用父代理凭证"]
        POOL -->|"是"| ACQ["acquire_lease()\n策略: fill_first / round_robin / random / least_used"]
        ACQ --> FILT["过滤 exhausted 凭证\n(冷却中的凭证不参与选择)"]
        FILT --> LEASE_ID["选择凭证，返回 lease_id"]
        LEASE_ID --> CUR["current() 返回 PooledCredential 对象"]
        CUR --> SWAP["_swap_credential(entry)\nself.api_key = entry.runtime_api_key\nself.base_url = entry.runtime_base_url"]
        SWAP --> RUN
        SKIP --> RUN

        RUN["child.run_conversation()"]
        RUN --> ERR{"API 错误？"}
        ERR -->|"否"| OK["request_count += 1"]
        ERR -->|"429 / 402"| EXH["标记 exhausted\n设置冷却时间 1小时"]
        ERR -->|"其他错误"| LERR["记录 last_error_code"]
        OK & EXH & LERR --> REL["release_lease(lease_id)\n标记凭证为可用"]
    end
...
```mermaid
flowchart TD
    subgraph MAIN["主线程 (_run_single_child)"]
        M1["创建 stop_event = threading.Event()"] --> M2
        M2["启动 _heartbeat_thread\ndaemon=True"] --> M3
        M3["child.run_conversation()\n阻塞等待子代理执行完毕"]
        M3 --> M4{"子代理完成或异常？"}
        M4 --> M5["stop_event.set()\n通知心跳线程停止"]
        M5 --> M6["heartbeat_thread.join(timeout=5s)\n等待线程退出"]
    end

    subgraph HEARTBEAT["心跳线程 (_heartbeat_loop) — 每 30s 触发一次"]
        H1(["线程启动"]) --> H2
        H2{"stop_event.wait\n(_HEARTBEAT_INTERVAL=30s) 超时？"}
        H2 -->|"stop_event 被 set"| HEXIT(["线程退出"])
        H2 -->|"超时，继续心跳"| H3
        H3{"parent._touch_activity\n存在？"}
        H3 -->|"否"| H2
        H3 -->|"是"| H4
        H4["child.get_activity_summary()\n获取当前子代理状态"]
        H4 --> H5{"summary.current_tool\n非空？"}
        H5 -->|"是"| DESC1["desc = 'delegate_task: subagent\nrunning {tool} (iter N/M)'"]
        H5 -->|"否"| DESC2["desc = 'delegate_task: subagent\n{last_activity_desc} (iter N/M)'"]
        DESC1 & DESC2 --> TOUCH["parent._touch_activity(desc)\n→ 更新父代理 _last_activity_ts\n→ 网关视角：父代理仍"活跃"\n→ 不触发不活动超时"]
        TOUCH --> H2
    end

    subgraph EFFECT["效果对比"]
        direction LR
        EF1["❌ 无心跳\n父代理 _last_activity_ts 冻结\n网关超时检测触发 → 终止父代理"]
        EF2["✅ 有心跳\n每 30s 触发 _touch_activity\n网关持续看到父代理'活跃'\n→ 子代理安全完成任务"]
    end
