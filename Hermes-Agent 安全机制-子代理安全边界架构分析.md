...
```mermaid
flowchart TD
    Start[LLM 调用 delegate_task] --> ValidateParent{parent_agent<br/>存在？}
    
    ValidateParent -->|否 | ReturnError1[返回错误<br/>"No parent agent"]
    ValidateParent -->|是 | CheckDepth{depth<br/>>= MAX_DEPTH?}
    
    CheckDepth -->|是 | ReturnError2[返回错误<br/>"Depth limit reached"]
    CheckDepth -->|否 | ValidateInput{goal/tasks<br/>非空？}
    
    ValidateInput -->|否 | ReturnError3[返回错误<br/>"Goal or tasks required"]
    ValidateInput -->|是 | CheckConcurrency{len tasks<br/><= 3?}
    
    CheckConcurrency -->|是 | LoadConfig[加载 delegation<br/>config.yaml]
    CheckConcurrency -->|否 | ReturnError4[返回错误<br/>"Too many tasks"]
    
    LoadConfig --> ResolveCreds[_resolve_delegation_<br/>credentials]
    
    ResolveCreds --> HasProvider{delegation.<br/>provider 指定？}
    HasProvider -->|是 | ResolveProvider[解析 provider<br/>base_url/api_key]
    HasProvider -->|否 | InheritParent[继承父代理凭证]
    
    ResolveProvider --> StripTools
    InheritParent --> StripTools
    
    StripTools[_strip_blocked_<br/>tools toolsets] --> RemoveSensitive[移除 delegate_task/<br/>clarify/memory/execute_code/...]
    
    RemoveSensitive --> BuildChildren[构建子代理列表]
    
    BuildChildren --> ForEachTask{遍历 tasks}
    
    ForEachTask --> SaveToolNames[保存父代理<br/>_last_resolved_tool_names]
    SaveToolNames --> CreateChild[_build_child_agent<br/>task_index, goal,<br/>context, toolsets,<br/>creds, depth+1]
    
    CreateChild --> SetIsolation[设置隔离:<br/>• 工具集白名单<br/>• 凭证池<br/>• 深度 +1<br/>• 回调继承]
    
    SetIsolation --> NextTask{还有任务？}
    NextTask -->|是 | ForEachTask
    NextTask -->|否 | CheckSingleTask{n_tasks == 1?}
    
    CheckSingleTask -->|是 | RunSingle[直接调用<br/>_run_single_child]
    CheckSingleTask -->|否 | RunParallel[ThreadPoolExecutor<br/>并发执行最多 3 个]
    
    RunSingle --> CollectResults[收集结果]
    RunParallel --> CollectResults
    
    CollectResults --> RestoreToolNames[恢复父代理<br/>_last_resolved_tool_names]
    
    RestoreToolNames --> FormatResponse[格式化响应<br/>results +<br/>total_duration +<br/>total_api_calls]
    
    FormatResponse --> ReturnJSON[返回 JSON 结果]
...
```mermaid
flowchart TD
    Start[_resolve_delegation_<br/>credentials cfg, parent] --> CheckProvider{cfg.<br/>provider 非空？}
    
    CheckProvider -->|否 | CheckModel{cfg.<br/>model 非空？}
    CheckProvider -->|是 | CheckDirectEndpoint{cfg.<br/>base_url<br/>指定？}
    
    CheckModel -->|否 | ReturnNone[返回全 None<br/>继承父代理]
    CheckModel -->|是 | ReturnModelOnly[返回 model<br/>其他 None]
    
    CheckDirectEndpoint -->|是 | UseCustomProvider[provider="custom"<br/>使用 cfg.base_url/<br/>cfg.api_key 或<br/>OPENAI_API_KEY env]
    CheckDirectEndpoint -->|否 | ResolveRuntimeProvider[调用 resolve_<br/>runtime_provider<br/>cfg.provider]
    
    ResolveRuntimeProvider --> ResolveSuccess{解析<br/>成功？}
    ResolveSuccess -->|否 | RaiseError[抛出 ValueError<br/>"Cannot resolve provider"]
    ResolveSuccess -->|是 | CheckApiKey{resolved.<br/>api_key 非空？}
    
    CheckApiKey -->|否 | RaiseError2[抛出 ValueError<br/>"no API key"]
    CheckApiKey -->|是 | UseResolved[使用 resolved<br/>provider/base_url/<br/>api_key/api_mode]
    
    UseCustomProvider --> ReturnCreds
    UseResolved --> ReturnCreds
    
    ReturnNone --> ReturnCreds[返回凭证字典]
    ReturnModelOnly --> ReturnCreds
    RaiseError --> End
    RaiseError2 --> End
    ReturnCreds --> End
    
    End --> StartLease[_run_single_child<br/>凭证租赁流程]
    
    StartLease --> CheckPool{child.<br/>_credential_pool<br/>存在？}
    
    CheckPool -->|否 | SkipLease[跳过租赁<br/>使用父代理凭证]
    CheckPool -->|是 | AcquireLease[acquire_lease]
    
    AcquireLease --> SelectStrategy[选择策略:<br/>fill_first/<br/>round_robin/<br/>random/<br/>least_used]
    
    SelectStrategy --> FilterExhausted[过滤 exhausted<br/>凭证]
    
    FilterExhausted --> SelectCred[选择凭证<br/>返回 lease_id]
    
    SelectCred --> GetCurrent[current<br/>返回 PooledCredential]
    
    GetCurrent --> SwapCred[_swap_credential<br/>切换到租赁凭证]
    
    SwapCred --> RunConv[run_conversation]
    
    RunConv --> TrackRequest[跟踪请求:<br/>request_count += 1]
    
    TrackRequest --> CheckAPIError{API<br/>错误？}
    
    CheckAPIError -->|否 | Continue
    CheckAPIError -->|是 429/402| MarkExhausted[标记 exhausted<br/>设置冷却时间]
    CheckAPIError -->|是 其他 | MarkError[标记 last_error_*]
    
    MarkExhausted --> ReleaseLease
    MarkError --> ReleaseLease
    Continue --> ReleaseLease
    
    ReleaseLease[release_lease<br/>lease_id] --> EndLease[结束租赁]
...
```mermaid
flowchart TD
    Start[_run_single_child<br/>启动心跳线程] --> CreateStopEvent[创建 threading.Event<br/>stop_heartbeat]
    
    CreateStopEvent --> DefineHeartbeat[定义 _heartbeat_loop]
    
    DefineHeartbeat --> CheckStop{stop_<br/>heartbeat<br/>is_set?}
    
    CheckStop -->|是 | StopThread[停止线程]
    CheckStop -->|否 | GetSummary[child.get_<br/>activity_summary]
    
    GetSummary --> ExtractDesc{summary.<br/>current_tool<br/>非空？}
    
    ExtractDesc -->|是 | UseToolDesc[desc =<br/>"executing tool:<br/>{current_tool}"]
    ExtractDesc -->|否 | UseLastDesc[desc =<br/>summary.<br/>last_activity_desc]
    
    UseToolDesc --> TouchParent[parent._touch_<br/>activity<br/>"child #{i} {desc}"]
    UseLastDesc --> TouchParent
    
    TouchParent --> WaitEvent[stop_heartbeat.<br/>wait<br/>_HEARTBEAT_INTERVAL=0.1s]
    
    WaitEvent --> CheckStop
    
    StopThread --> StartThread[启动后台线程<br/>heartbeat_thread =<br/>Thread target=_heartbeat_loop]
    
    StartThread --> RunConversation[执行 child.<br/>run_conversation]
    
    RunConversation --> CheckComplete{child<br/>完成？}
    
    CheckComplete -->|否 | WaitConv[等待完成]
    CheckComplete -->|是 | SetStop[stop_heartbeat.<br/>set]
    
    SetStop --> JoinThread[join heartbeat_thread<br/>timeout=1s]
    
    JoinThread --> EndHeartbeat[结束心跳]
    
    WaitConv --> CheckComplete
```
