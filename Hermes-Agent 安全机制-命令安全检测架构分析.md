...
```mermaid
flowchart TD
    Start[开始] --> CheckEnv{env_type 是<br/>docker/singularity/<br/>modal/daytona?}
    
    CheckEnv -->|是 | SkipCheck[跳过检查<br/>直接执行]
    CheckEnv -->|否 | CheckYolo{YOLO 模式<br/>或 approvals.mode=off?}
    
    CheckYolo -->|是 | SkipCheck
    CheckYolo -->|否 | Phase1[Phase 1: 收集发现]
    
    Phase1 --> TirithScan[调用 Tirith 扫描<br/>check_command_security]
    TirithScan --> TirithResult{Tirith verdict}
    
    TirithResult -->|allow| CheckDangerous
    TirithResult -->|warn/block| CollectTirith[收集 Tirith findings<br/>tirith_key = "tirith:rule_id"]
    
    CheckDangerous[调用 detect_dangerous_<br/>command] --> IsDangerous{匹配危险<br/>模式?}
    
    IsDangerous -->|否 | CheckWarnings{有任何<br/>警告?}
    IsDangerous -->|是 | CollectDangerous[收集危险模式<br/>pattern_key, description]
    
    CollectDangerous --> CheckWarnings
    CollectTirith --> CheckWarnings
    
    CheckWarnings -->|无 | AllowExec[允许执行]
    CheckWarnings -->|有 | Phase2[Phase 2: 决策]
    
    Phase2 --> CheckApproved{检查会话/<br/>永久审批}
    
    CheckApproved -->|已审批 | AllowExec
    CheckApproved -->|未审批 | CheckSmartMode{approvals.<br/>mode=smart?}
    
    CheckSmartMode -->|是 | LLMAssess[调用辅助 LLM<br/>风险评估]
    LLMAssess --> LLMVerdict{LLM  verdict}
    
    LLMVerdict -->|APPROVE| AutoApprove[自动批准<br/>授予会话级审批]
    LLMVerdict -->|DENY| BlockCmd[阻止命令<br/>返回 BLOCKED 消息]
    LLMVerdict -->|ESCALATE| PromptUser
    
    CheckSmartMode -->|manual| PromptUser[提示用户审批]
    
    PromptUser --> GatewayCheck{Gateway<br/>会话?}
    
    GatewayCheck -->|是 | GatewayQueue[加入审批队列<br/>阻塞等待]
    GatewayQueue --> NotifyUser[通知用户<br/>/approve /deny]
    NotifyUser --> ResolveApproval[用户响应<br/>once/session/always/deny]
    
    GatewayCheck -->|否 | CLIPrompt[CLI 交互式提示<br/>[o]nce/[s]ession/[a]lways/[d]eny]
    CLIPrompt --> ResolveApproval
    
    ResolveApproval --> Choice{用户选择}
    
    Choice -->|once| AllowOnce[允许一次<br/>不持久化]
    Choice -->|session| AllowSession[会话级允许<br/>approve_session]
    Choice -->|always| AllowAlways[永久允许<br/>approve_permanent<br/>save_config]
    Choice -->|deny| BlockCmd
    
    AllowOnce --> AllowExec
    AllowSession --> AllowExec
    AllowAlways --> AllowExec
    AutoApprove --> AllowExec
    
    AllowExec --> ExecuteCommand[执行命令]
    BlockCmd --> ReturnBlocked[返回 BLOCKED 消息]
    
    ExecuteCommand --> End[结束]
    ReturnBlocked --> End
    SkipCheck --> ExecuteCommand
```
...
```mermaid
flowchart TD
    Start[check_command_<br/>security command] --> LoadConfig[加载配置<br/>tirith_enabled/path/<br/>timeout/fail_open]
    
    LoadConfig --> CheckEnabled{tirith_<br/>enabled?}
    
    CheckEnabled -->|否 | ReturnAllow[返回 allow]
    CheckEnabled -->|是 | ResolveBinary[解析 Tirith 二进制<br/>_resolve_tirith_binary]
    
    ResolveBinary --> CheckCached{已缓存<br/>_resolved_path?}
    CheckCached -->|是 | UseCached[使用缓存路径]
    CheckCached -->|否 | CheckPATH[检查 PATH 中的<br/>tirith 二进制]
    
    CheckPATH --> FoundInPATH{在 PATH<br/>中找到?}
    FoundInPATH -->|是 | UsePATH[使用 PATH 中的路径]
    FoundInPATH -->|否 | CheckConfigPath{检查配置<br/>tirith_path}
    
    CheckConfigPath --> Exists{文件<br/>存在?}
    Exists -->|是 | UseConfigPath[使用配置路径]
    Exists -->|否 | BackgroundInstall[后台线程安装]
    
    BackgroundInstall --> DetectTarget[检测目标平台<br/>Linux/macOS x86_64/aarch64]
    DetectTarget --> Download[下载 GitHub<br/>Release tarball]
    Download --> VerifySHA256[验证 SHA-256<br/>校验和]
    VerifySHA256 --> CheckCosign{cosign<br/>在 PATH 中?}
    
    CheckCosign -->|是 | CosignVerify[cosign 签名<br/>验证]
    CheckCosign -->|否 | SkipCosign[跳过 cosign 验证]
    
    CosignVerify --> Extract[解压到<br/>$HERMES_HOME/bin/]
    SkipCosign --> Extract
    
    Extract --> SetPerms[chmod +x<br/>设置执行权限]
    SetPerms --> CachePath[缓存到<br/>_resolved_path]
    
    UseCached --> ExecuteScan
    UsePATH --> ExecuteScan
    UseConfigPath --> ExecuteScan
    CachePath --> ExecuteScan
    
    ExecuteScan[执行 tirith scan<br/>subprocess.run] --> CheckTimeout{超时<br/>或错误?}
    
    CheckTimeout -->|是 | CheckFailOpen{fail_<br/>open?}
    CheckFailOpen -->|是 | ReturnAllow
    CheckFailOpen -->|否 | ReturnBlock[返回 block]
    
    CheckTimeout -->|否 | ParseOutput[解析 JSON 输出<br/>findings + summary]
    ParseOutput --> CheckExitCode{退出码}
    
    CheckExitCode -->|0| ReturnAllow
    CheckExitCode -->|1| ReturnBlock
    CheckExitCode -->|2| ReturnWarn[返回 warn]
...
```mermaid
flowchart TD
    Start[detect_dangerous_<br/>command command] --> Normalize[命令规范化<br/>_normalize_command_<br/>for_detection]
    
    Normalize --> StripANSI[strip_ansi<br/>ANSI 转义序列清洗]
    StripANSI --> RemoveNull[移除 null 字节]
    RemoveNull --> NormalizeUnicode[Unicode NFKC<br/>规范化]
    
    NormalizeUnicode --> LowerCase[转换为小写]
    LowerCase --> IteratePatterns[遍历 DANGEROUS_<br/>PATTERNS 130+]
    
    IteratePatterns --> MatchPattern{re.search<br/>pattern, command}
    
    MatchPattern -->|匹配 | ReturnDangerous[返回<br/>True, pattern_key,<br/>description]
    MatchPattern -->|不匹配 | NextPattern{还有<br/>模式?}
    
    NextPattern -->|是 | IteratePatterns
    NextPattern -->|否 | ReturnSafe[返回<br/>False, None, None]
    
    ReturnDangerous --> End[结束]
    ReturnSafe --> End
```
...
```mermaid
flowchart TD
    Start[_smart_approve<br/>command, description] --> GetAuxClient[获取辅助 LLM 客户端<br/>get_text_auxiliary_client]
    
    GetAuxClient --> CheckAvailable{LLM 客户端<br/>可用?}
    
    CheckAvailable -->|否 | ReturnEscalate[返回 escalate]
    CheckAvailable -->|是 | BuildPrompt[构建风险评估提示词]
    
    BuildPrompt --> PromptText["You are a security reviewer...
