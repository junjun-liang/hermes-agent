...
│                          命令执行流程                                         │
│                                                                             │
│  1. 入口: env.execute(command, timeout, login)                              │
│     │                                                                       │
│     ▼                                                                       │
│  ┌──────────────────────────────────────────────────────────────────┐      │
│  │ _before_execute()                                                │      │
│  │                                                                  │      │
│  │ • 检查是否需要文件同步 (FileSyncManager.sync)                    │      │
│  │ • Docker: 检查容器是否运行，必要时重启                           │      │
│  │ • SSH: 检查 ControlMaster 连接                                  │      │
│  │ • Modal: 检查沙箱是否存活                                       │      │
│  └───────────────────────────────┬──────────────────────────────────┘      │
│                                  │                                        │
│                                  ▼                                        │
│  ┌──────────────────────────────────────────────────────────────────┐      │
│  │ _wrap_command(command) — 命令包装                                │      │
│  │                                                                  │      │
│  │ ┌──────────────────────────────────────────────────────────┐    │      │
│  │ │ {                                                          │    │      │
│  │ │   source /tmp/snapshot.json && \\                          │    │      │
│  │ │   cd /target/cwd && \\                                     │    │      │
│  │ │   echo "___HERMES_CWD_START___" && pwd && \\              │    │      │
│  │ │   echo "___HERMES_CWD_END___" && \\                       │    │      │
...
│  │ └──────────────────────────────────────────────────────────┘    │      │
│  │                                                                  │      │
│  │ • source snapshot.json: 恢复上次捕获的环境变量                  │      │
│  │ • cd: 切换到目标工作目录                                        │      │
│  │ • CWD 标记: 命令前后各输出一次 pwd，用于追踪 CWD 变化            │      │
│  └───────────────────────────────┬──────────────────────────────────┘      │
│                                  │                                        │
│                                  ▼                                        │
│  ┌──────────────────────────────────────────────────────────────────┐      │
│  │ _run_bash(wrapped_command, login, timeout)                       │      │
│  │                                                                  │      │
│  │ 根据 env_type 分发到不同后端:                                     │      │
│  │                                                                  │      │
│  │ ┌─────────┬──────────────────────────────────────────────┐     │      │
...
│  │ ├─────────┼──────────────────────────────────────────────┤     │      │
│  │ │ ssh     │ ssh -o ControlPath=... user@host bash -c cmd │     │      │
│  │ ├─────────┼──────────────────────────────────────────────┤     │      │
│  │ │ modal   │ Sandbox.exec(["bash", "-c", cmd])            │     │      │
│  │ ├─────────┼──────────────────────────────────────────────┤     │      │
│  │ │ daytona │ sandbox.process.exec(cmd)                    │     │      │
│  │ ├─────────┼──────────────────────────────────────────────┤     │      │
│  │ │singularity│ apptainer exec instance:// bash -c cmd    │     │      │
│  │ └─────────┴──────────────────────────────────────────────┘     │      │
│  └───────────────────────────────┬──────────────────────────────────┘      │
│                                  │                                        │
│                                  ▼                                        │
...
│  │   ├── 每 10 秒: activity_callback()  ← 防止网关超时              │      │
│  │   ├── 检查超时: 超时 → proc.kill() + SIGTERM → SIGKILL          │      │
│  │   ├── 检查中断: 用户中断 → proc.kill()                           │      │
│  │   └── 否则: sleep(0.1) 继续等待                                  │      │
│  │                                                                  │      │
│  │ 返回: {output, exit_code}                                        │      │
...
│  │                                                                  │      │
│  │ 从 stdout 提取 CWD:                                              │      │
│  │   正则: ___HERMES_CWD_START___\\n(.+?)\\n___HERMES_CWD_END___   │      │
│  │   → 更新 self.cwd                                                │      │
│  └───────────────────────────────┬──────────────────────────────────┘      │
│                                  │                                        │
│                                  ▼                                        │
│  ┌──────────────────────────────────────────────────────────────────┐      │
│  │ _after_execute(result)                                           │      │
│  │                                                                  │      │
│  │ • 记录执行结果                                                   │      │
│  │ • 更新执行统计                                                   │      │
│  └───────────────────────────────┬──────────────────────────────────┘      │
...
│                        环境变量隔离流程                                        │
│                                                                             │
│  构建子进程环境: _sanitize_subprocess_env(base_env, extra_env)              │
│     │                                                                       │
│     ▼                                                                       │
│  ┌──────────────────────────────────────────────────────────────────┐      │
│  │ 步骤 1: 加载黑名单                                               │      │
│  │                                                                  │      │
│  │ _HERMES_PROVIDER_ENV_BLOCKLIST (100+ 项)                        │      │
│  │                                                                  │      │
│  │ API Key 前缀:                                                    │      │
│  │   OPENAI_API_KEY, ANTHROPIC_API_KEY, GOOGLE_API_KEY, ...        │      │
│  │                                                                  │      │
│  │ Token 前缀:                                                      │      │
│  │   GITHUB_TOKEN, MODAL_TOKEN_ID, DAYTONA_API_KEY, ...            │      │
│  │                                                                  │      │
│  │ Secret 前缀:                                                     │      │
│  │   *_SECRET, *_PASSWORD, *_CREDENTIAL                            │      │
│  │                                                                  │      │
│  │ Messaging:                                                       │      │
│  │   TELEGRAM_*, DISCORD_*, SLACK_*, SIGNAL_*, ...                 │      │
│  │                                                                  │      │
│  │ Provider:                                                        │      │
│  │   HERMES_PROVIDER_*, PROVIDER_*, MODEL_*, ...                   │      │
│  └───────────────────────────────┬──────────────────────────────────┘      │
│                                  │                                        │
│                                  ▼                                        │
│  ┌──────────────────────────────────────────────────────────────────┐      │
│  │ 步骤 2: 遍历 os.environ，逐项判断                               │      │
│  │                                                                  │      │
│  │ 对于每个 key:                                                    │      │
│  │                                                                  │      │
│  │  ┌────────────────────────────────────────────────────────┐     │      │
...
│  │        ▼               │                                         │      │
│  │  剥离前缀:          ┌──────────────────────────────────────┐    │      │
│  │  _HERMES_FORCE_X   │ is_env_passthrough(key)?              │    │      │
│  │  → X             └────────────┬───────────────────────────┘    │      │
│  │  强制加入             ┌───────┴───────┐                        │      │
│  │                       ▼               ▼                        │      │
│  │                      是               否                        │      │
...
│  │ 步骤 3: 添加特殊变量                                             │      │
│  │                                                                  │      │
│  │ PATH: 如果未设置，添加 sane default                              │      │
│  │       /usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin              │      │
│  │                                                                  │      │
│  │ HOME: 重定向到 Profile 隔离目录                                  │      │
│  │       get_subprocess_home() → {HERMES_HOME}/home/               │      │
│  │       防止子进程读取 ~/.ssh, ~/.config 等敏感配置               │      │
│  │                                                                  │      │
│  │ PYTHONDONTWRITEBYTECODE=1  ← 避免生成 .pyc                      │      │
│  │ PYTHONPATH: 清理 (防止注入恶意模块)                             │      │
│  │ TZ: 时区设置                                                     │      │
│  └───────────────────────────────┬──────────────────────────────────┘      │
│                                  │                                        │
│                                  ▼                                        │
│  ┌──────────────────────────────────────────────────────────────────┐      │
│  │ 步骤 4: 返回 sanitized_env                                       │      │
│  │                                                                  │      │
│  │ 结果:                                                            │      │
│  │   • 100+ 密钥/凭证/配置变量被排除                                │      │
│  │   • 技能声明的变量 (register_env_passthrough) 被透传            │      │
│  │   • 用户配置的变量 (config.yaml terminal.env_passthrough) 透传  │      │
│  │   • HOME 被重定向到 Profile 隔离目录                             │      │
│  │   • PATH 被设置为安全默认值                                      │      │
│  └──────────────────────────────────────────────────────────────────┘      │
│                                                                             │
│  透传机制详情:                                                              │
│  ┌──────────────────────────────────────────────────────────────────┐      │
│  │ is_env_passthrough(key) 检查:                                   │      │
│  │                                                                  │      │
│  │ 1. 技能声明:                                                     │      │
│  │    skill YAML frontmatter:                                       │      │
│  │    required_environment_variables: [API_KEY, CONFIG_PATH]       │      │
