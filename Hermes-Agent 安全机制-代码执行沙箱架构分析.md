...
```mermaid
flowchart TD
    Start[遍历 os.environ] --> CheckPassthrough{变量在<br/>env_passthrough<br/>白名单？}
    CheckPassthrough -->|是 | Include[包含在子进程环境]
    CheckPassthrough -->|否 | CheckSecret{变量名包含<br/>KEY/TOKEN/SECRET/<br/>PASSWORD/CREDENTIAL/AUTH?}
    CheckSecret -->|是 | Exclude[排除：不传递给子进程]
    CheckSecret -->|否 | CheckPrefix{变量名以安全<br/>前缀开头？}
    CheckPrefix -->|是 | Include
    CheckPrefix -->|否 | Exclude
    
    Include --> AddSpecial[添加特殊变量:<br/>HERMES_RPC_SOCKET<br/>PYTHONDONTWRITEBYTECODE<br/>PYTHONPATH<br/>TZ (时区)<br/>HOME (Profile 隔离)]
    AddSpecial --> End[构建 child_env 字典]
    Exclude --> End
```
...
```mermaid
flowchart LR
    RawOutput[原始 stdout/stderr] --> Truncate[Head/Tail 截断<br/>上限 50KB<br/>40% Head + 60% Tail]
    Truncate --> ANSIStrip[ANSI 转义序列清洗<br/>ECMA-48 全覆盖]
    ANSIStrip --> Redact[凭证脱敏<br/>API Key / Token / 私钥<br/>DB 连接串 / 手机号]
    Redact --> BuildJSON[构建 JSON 响应<br/>status + output +<br/>tool_calls_made + duration]
    BuildJSON --> Return[返回给 LLM]
```
...
stateDiagram-v2
    ...
    Running --> Running: 轮询 proc.poll()
    Running --> Exited: 正常退出
    Running --> Timeout: time.monotonic() > deadline
    Running --> Interrupted: is_interrupted() == True
    
    Timeout --> SIGTERM: _kill_process_group()
    SIGTERM --> WaitExit: proc.wait(5s)
    WaitExit --> SIGKILL: 超时未退出
    WaitExit --> CleanExit: 已退出
    
    Interrupted --> SIGTERM: _kill_process_group()
    
    SIGKILL --> CleanExit
    CleanExit --> Cleanup
    Exited --> Cleanup
    
    Cleanup --> [*]: 删除临时目录 + Socket
```
