...
description: 克隆、创建、Fork、配置和管理 GitHub 仓库。管理远程仓库、Secrets、发布和工作流。使用 gh CLI 或回退到 git + 通过 curl 调用 GitHub REST API。
version: 1.1.0
author: Hermes Agent
license: MIT
metadata:
...
创建、克隆、Fork、配置和管理 GitHub 仓库。每个部分先展示 `gh` 用法，然后是 `git` + `curl` 回退方案。

## 前置条件

- 已完成 GitHub 身份验证（参见 `github-auth` 技能）
...
Secrets 需要使用仓库公钥加密 — 通过 API 操作更复杂：
...
注意：对于 Secrets，`gh secret set` 要简单得多。如果需要设置 Secrets 但 `gh` 不可用，建议仅为该操作安装它。
